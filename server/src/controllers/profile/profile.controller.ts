...
export async function AttachGoogleLogin(req: Request, res: Response) {
    ...
    try {
        ...
        const user = await User.findByIdAndUpdate(req.userid, { provider: "hybrid",  user_provider_id: payload?.sub,email_verified:true }).select("password provider email email_verified")
        SuccessResponse(res, { payload: user })
    }
    catch (err) {
        ErrorResponse(res, { message: "Internal server error ,Try again later !" })
