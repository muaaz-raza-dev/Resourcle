...
export default function useSearchResource() {
  ...
  return useMutation({
    ...
    mutationFn: (directPayload?:{search?:string,sort?:SearchedSortOptions}) => SearchResourceApi({count,search:directPayload?.search||search,sort:directPayload?.sort||sortP||"upvotes"}),
    onSuccess({payload:{resources,total}}) {
      setValue(val=>({...val,resources:{...val.resources,[count]:resources},total}))
    },
    onError({
      response: {
        data: { message },
