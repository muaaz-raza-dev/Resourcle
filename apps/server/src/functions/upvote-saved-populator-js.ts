...
export async function UpvoteAndSavedPopulator(req:Request,{resource,resourceRaw,isNestedUpvote}:{resourceRaw:IResource,resource:IResourcePayload,isNestedUpvote?:boolean}){
    ...
    if(isNestedUpvote){
        const content_upvotes = await Upvotes.aggregate([
            ...
            {$match:{"content_votes.users":new mongoose.Types.ObjectId(req.userid)}},
            {$project:{link_id:"$content_votes.link_id"}}
        ]) as {link_id:string;}[]
            resource.content.forEach((content,contentIndex)=>{
                content.links.forEach((l,lIndex)=>{
                        resource.content[contentIndex].links[lIndex].isUpvoted =content_upvotes.length? content_upvotes.some(cv=>cv.link_id.toString()==l._id.toString()) : false
                })
        })
}
    return resource
