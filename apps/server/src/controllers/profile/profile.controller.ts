...
export async function UpdateProfileInfoController(req: Request, res: Response) {
    try {
        ...
        const user_details = await User.findByIdAndUpdate(req.userid, { name, links, about, headline, picture, username }, { new: true })
...
export async function ValidateUsername(req: Request, res: Response) {
    try {
        ...
        if (!user) {
            SuccessResponse(res, { message: "username available", payload: { isAvailable: true } });
            return;
        }
        SuccessResponse(res, { message: "username is taken. try another", payload: { isAvailable: false } });
        return;
    }
    catch (error) {
        ErrorResponse(res, { message: "Internal Server Error", status: 500, });
...
export async function GetSecurityInfo(req: Request, res: Response) {
    const user = await User.findById(req.userid).select("provider email email_verified reset_verification reset_token_expiration")
...
export async function ChangePassword(req: Request, res: Response) {
    ...
    try {
        const user = await User.findById(req.userid).select("password reset_verification reset_token_expiration")
        if(!user) {
            ErrorResponse(res,{message:"Invalid credentials",status:401})
            return;
        }
        if (req.details.password) {
            if(!user?.reset_verification){
                const isValid = await bcrpyt.compare(current_password, req.details.password)
                if (!isValid) {
                    ErrorResponse(res, { message: "Current password is invalid", status: 401 })
                    return;
                }
                else if(moment(user.reset_token_expiration).isBefore(moment())) {
                    ErrorResponse(res, { message: "Times up! need to request otp again !", status: 401 })
                    return;
                }
            }
        }
      ...
      await User.findByIdAndUpdate(req.userid, { provider,reset_verification:false , password: encryptedPassword
            ,...(provider=="local"?{$unset:{user_provider_id:""}}:{}) }).select("password provider email email_verified")
...
export async function AttachGoogleLogin(req: Request, res: Response) {
    ...
    try {
        if(provider=="local"){
            ErrorResponse(res,{message:"Sorry You are at wrong place",status:403})
            return;
        }        
        ...
        const user = await User.findByIdAndUpdate(req.userid, { provider ,user_provider_id: payload?.sub,email_verified:true }).select("password provider email email_verified")
        SuccessResponse(res, { payload: user })
    }
    catch (err) {
        ErrorResponse(res, { message: "Internal server error ,Try again later !" })
...
export async function DetachHybridLoginProvider(req:Request,res:Response){
    ...
    if(currentProvider==provider){
        ...
        const updateQuery = provider=="local" ? {provider, $unset:{user_provider_id:1}} : {provider, $unset:{password:1}}
        await User.findByIdAndUpdate(req.userid, updateQuery).select("password provider email email_verified")
        SuccessResponse(res,{message:`You're are now in ${provider} login mode`})        
        return;
    }
    else{
