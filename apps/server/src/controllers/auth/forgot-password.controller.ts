...
export async function VerifyOTP(req: Request, res: Response) {
  ...
  if (
    !user.reset_token ||
    moment().isAfter(moment(user.reset_token_expiration))
  ) {
  ...
  const updated_user = await User.findByIdAndUpdate(
    user._id,
    { $unset: { reset_token: 1 }, $set: { reset_verification: true } },
    { new: true },
  );
  const login_token = jwt.sign({ user_id: user._id }, JWT_SECRET || "", {
    expiresIn: "30d",
  });
  res
