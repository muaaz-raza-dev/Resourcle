    ...
    <section className="flex flex-col gap-2 rounded-md ">
            ...
            <div className="flex  gap-1  items-center ">
              ...
              >
                <Tooltip title="Add new link" placement="top">
                  <button
                    type="button"
                    className=" border text-secondary-foreground font-bold text-sm rounded-md p-2  aspect-square center hover:bg-primary/40 transition-colors"
                  >
                    <FaPlus />
                  </button>
                </Tooltip>
              ...
              <div className="flex justify-between hover:rounded transition-all w-full rounded-md p-3">
                <div className="flex max-md:flex-col md:items-center md:gap-2">
                  <p className=" cursor-default text-sm font-semibold transition-colors">
                    {link.title}
                  </p>
                  <a
                    href={link.url}
                    target="_blank"
                    className="text-primary text-sm md:border-l md:px-3 text-wrap"
                  >
                    {link.url}
                  </a>
                </div>
      ...
      <div className="center">
        ...
        >
          <button
            type="button"
            className=" bg-primary/30 hover:bg-primary/40 w-max text-black  transition-colors px-4 py-1 font-semibold rounded-md text-sm  center gap-1"
          >
            Add link
          </button>
        </NewLinkDailogResourceForm>
...
function DetailsAndActionsLinkSection({
  index,
  link,
  link_index,
}: {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        side="bottom"
        align="center"
        className="max-md:w-4xl mr-4"
      >
        <a
          href={link.url}
          target="_blank"
          className="text-primary underline text-wrap break-all  text-sm w-full"
        >
          <p>{link.url}</p>
        </a>
        ...
        <div className="flex gap-2 flex-wrap pt-4 text-sm border-t">
          <NewLinkDailogResourceForm
            data={link}
            linkGroupIndex={index}
            ...
            isEdit
          >
            <button className="flex gap-2  border items-center px-4 py-1 bg-secondary hover:bg-border font-semibold transition-colors rounded-md ">
          ...
          <button
            type="button"
            onClick={() => DeleteLink(link_index)}
            className="flex py-1  items-center gap-2  border px-4  bg-secondary font-semibold hover:bg-border transition-colors text-destructive   rounded-md p-2"
          >
            Delete <FaTrash />
          </button>
          <SwitchLinkGroupResourceForm currentGroupIndex={index} linkIndex={link_index}/>
  ...
  return (
    <div className="center flex-col border rounded-md p-2 gap-1 py-8">
      <NewLinkDailogResourceForm linkGroupIndex={index} linkIndex={0}>
        <button
          type="button"
          className=" bg-primary/30 hover:bg-primary/40  text-black  transition-colors px-4 py-1 font-semibold rounded-md text-sm  center gap-1"
        >
          Add links
        </button>
      </NewLinkDailogResourceForm>
